...
## 2º) Comprobar si la carpeta no existe antes de crearla:
    Nombre_Lugar= Lugar[0].split(',')[0].strip().replace(' ', '_') + '_' + Tipo_Calles # Nombre del Lugar extraído del parámetro 'Lugar'.
    
    if not os.path.exists('./'): # Si no existe ya la carpeta...
        os.makedirs('./ANALISIS_CENTRALIDAD_{}'.format(Nombre_Lugar)) # Creala con el Nombre del Lugar a analizar.
##------------------------------------------------------------------------------------------------------------------##
## 3º) Guardar Grafo:
    if Guardar_Grafo==True:
        ox.save_graphml(G, './ANALISIS_CENTRALIDAD_{}/Grafo_{}.graphml'.format(Nombre_Lugar, Nombre_Lugar))
##=========================================================================================================================================##
##=========================================================================================================================================##
...
##------------------------------------------------------------------------------------------------------------------##
## 11º) Etiquetas de las aristas incidentes en el Nodo clave: ("set"-> EVITA DUPLICIDADES):
    etiquetas_aristas= []
...
## GUARDAR IMAGEN:
    if Guardar_img==True:
            plt.savefig('./ANALISIS_CENTRALIDAD_{}/Grafo_{}_Nodo_Mayor_Centralidad.png'.format(Nombre_Lugar, Nombre_Lugar))
##------------------------------------------------------------------------------------------------------------------##
## 12º) Crear MAPA INTERACTIVO con el NODO CENTRAL RESALTADO EN ROJO:
    ...
    gdf_nodes['color']= ['red' if node_id == Nodo_Clave else 'blue' for node_id in gdf_nodes.index]

    # c) Resaltar el Nodo_Clave con MAYOR TAMAÑO (y el resto más pequeño):
    gdf_nodes['geometry'] = gdf_nodes['geometry'].buffer(15)
    gdf_nodes.loc[gdf_nodes.index == Nodo_Clave, 'geometry'] = gdf_nodes.loc[gdf_nodes.index == Nodo_Clave, 'geometry'].buffer(30)

    # d) Mostrar como información el Nº de Nodo:
    gdf_nodes['Nº Node']= gdf_nodes.index

    # e) MAPA INTERACTIVO:    (Color_Nodos, Leyenda,     Estilo_Borde_Nodos,                        Estilo del Mapa,          Información al Pulsar, Resto de Info=NO):
    mapa_nodos = gdf_nodes.explore(color='color', legend=True, style_kwds={'color':'black','weight':0.3}, tiles='CartoDB positron', popup=['Nº Node'], tooltip=None)

    # f) Verificar si la carpeta 'MAPAS_INTERACTIVOS' ya existe dentro de 'ANALISIS_CENTRALIDAD':
    if not os.path.exists('./ANALISIS_CENTRALIDAD_{}/MAPAS_INTERACTIVOS'.format(Nombre_Lugar)):
        os.makedirs('./ANALISIS_CENTRALIDAD_{}/MAPAS_INTERACTIVOS'.format(Nombre_Lugar))  # Si no existe-> crearla.

    # g) Guardar el Mapa Interactivo como archivo HTML (Google):
    mapa_nodos.save('./ANALISIS_CENTRALIDAD_{}/MAPAS_INTERACTIVOS/MAPA_INTERACTIVO_Nodo_Mayor_Centralidad.html'.format(Nombre_Lugar))
##=========================================================================================================================================##
##=========================================================================================================================================##
...
## GUARDAR IMAGEN:
    if Guardar_img==True:
            plt.savefig('./ANALISIS_CENTRALIDAD_{}/Grafo_{}_Nodo_Mayor_Centralidad_ZOOM.png'.format(Nombre_Lugar, Nombre_Lugar))
##=========================================================================================================================================##
##=========================================================================================================================================##
...
## GUARDAR IMAGEN:
    if Guardar_img==True:
            plt.savefig('./ANALISIS_CENTRALIDAD_{}/Grafo_{}_Nodos_según_Centralidad_MEDIA.png'.format(Nombre_Lugar, Nombre_Lugar))
##------------------------------------------------------------------------------------------------------------------##
## 18º) Crear MAPA INTERACTIVO con los NODOS según su Grado de CENTRALIDAD MEDIA:     (Mapa_Color,     Leyenda,     Estilo_Bordes_Nodos,                       Estilo_Mapa,
    mapa_nodos_segun_centralidad = gdf_nodes.explore(column='Media_Centralidad', cmap='coolwarm', legend=True, style_kwds={'color':'black','weight':0.3}, tiles='CartoDB positron',
    ...
    mapa_nodos_segun_centralidad.save('./ANALISIS_CENTRALIDAD_{}/MAPAS_INTERACTIVOS/MAPA_INTERACTIVO_Nodos_según_Centralidad_MEDIA.html'.format(Nombre_Lugar))
##=========================================================================================================================================##
##=========================================================================================================================================##
...
## GUARDAR IMAGEN:
    ...
    if Guardar_img==True:
            plt.savefig('./ANALISIS_CENTRALIDAD_{}/Grafo_{}_Llegada_en_Tiempos_{}_minutos.png'.format(Nombre_Lugar, Nombre_Lugar, Tiempos))
##------------------------------------------------------------------------------------------------------------------##
## 26º) MAPA INTERACTIVO ISOCRONAS (Llegada en un determinado TIEMPO):
    ...
    gdf_nodes['Colores_Isocronas']= node_colors
    ...
    gdf_nodes['Tiempo de Llegada (min)'] = gdf_nodes['Colores_Isocronas'].apply(asignar_indice_por_color)

    # e) MAPA INTERACTIVO:           (Color_Nodos,               Leyenda,     Estilo_Borde_Nodos,        Estilo del Mapa,          Información al Pulsar,                  Resto de Info=NO):
    Mapa_ISOCRONAS= gdf_nodes.explore(color='Colores_Isocronas', legend=True, style_kwds={'color':None}, tiles='CartoDB positron', popup=['Nº Node', 'Tiempo de Llegada (min)'], tooltip=None)
